...
train_x, train_y = global_preprocessor.get_medium_train_sequences()
test_x, test_y = global_preprocessor.get_medium_test_sequences()
...
train_x, medium_scaler = scale(train_x, is_x_train=True)
train_y, medium_scaler = scale(train_y, medium_scaler, is_x=False)
test_x, medium_scaler  = scale(test_x, scaler=medium_scaler)
test_y, medium_scaler  = scale(test_y, scaler=medium_scaler, is_x=False)
...
def build_model(hp):
    ...
    model.add(tf.keras.layers.LSTM(units=hp.Int('medium_input', min_value=64, max_value=128, step=32), 
...
tuner = RandomSearch(
    ...
    project_name='medium_regresssion_tuning'
)
