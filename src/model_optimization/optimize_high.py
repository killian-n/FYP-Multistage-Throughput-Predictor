...
train_x, train_y = global_preprocessor.get_high_train_sequences()
test_x, test_y = global_preprocessor.get_high_test_sequences()
...
train_x, high_scaler = scale(train_x, is_x_train=True)
train_y, high_scaler = scale(train_y, high_scaler, is_x=False)
test_x, high_scaler  = scale(test_x, scaler=high_scaler)
test_y, high_scaler  = scale(test_y, scaler=high_scaler, is_x=False)
...
def build_model(hp):
    ...
    model.add(tf.keras.layers.LSTM(units=hp.Int('high_input', min_value=64, max_value=128, step=32), 
...
tuner = RandomSearch(
    ...
    project_name='high_regresssion_tuning'
)
