...
class WirelessDataPreProcessor:
    def __init__(self, dataframe, is_complete=True):
        self.__df = dataframe
        self.__no_of_features = len(self.__df.columns)
        self.__no_of_examples = self.__df.size
        self.__numeric_columns = ["SNR", "CQI", "RSSI", "NRxRSRP", "NRxRSRQ", "ServingCell_Distance"]
        self.__categorical_columns = list(set(self.__df.columns).difference(self.__numeric_columns))
        self.__group_columns = ["movement_type", "session"]
        self.__geo_columns = ["ServingCell_Lon", "ServingCell_Lat"]
    ...
    def __impute_missing_data(self, dataframe):
        dataframe[self.__geo_columns].ffill(inplace=True)
        scaler = StandardScaler()
        x_data = dataframe[self.__numeric_columns]
        ...
        dataframe[self.__numeric_columns] = imputed_array
        return dataframe
